...
sem_t mutex; // Definição do semáforo
...
// classe que simula uma tabela DB
class Cadastros{
    private:
        std::vector<Registro> contas; //vetor que contem as contas
    ...
    public:
        void criarConta(const std::string& nome, double saldo, int id) {
            ...
            novaConta.id = id;
            contas.push_back(novaConta); // coloca a conta dentro do vetor 
    }
 
        Registro* lerConta(int id) {
            for ( auto &conta : contas) {
                if (conta.id == id) {
                    return &conta;     //retorna um ponteiro que aponta para conta
                }
            }
            std::cout << "Conta com ID " << id << " não encontrada.\n";
            return nullptr; // Retorna um objeto Registro vazio
    }
    ...
    int threadDeposito(void* arg) {
        ...
        Registro* conta = cadastro->lerConta(1);
